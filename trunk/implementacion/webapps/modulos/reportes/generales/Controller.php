...
function getExcel($dbcon, $tipo, $datos){
	...
	if ($tipo == 'Requisiciones') {
		$tipoBusqueda = " tipo in ('N', 'A') ";
		if (!$datos->tipoRANorm && $datos->tipoRAAuto) {
			$tipoBusqueda = " tipo = 'A'";
		}
		if ($datos->tipoRANorm && !$datos->tipoRAAuto) {
			$tipoBusqueda = " tipo = 'N'";
		...
		WHERE ".$tipoBusqueda." AND r.fecha_registro BETWEEN '".$datos->f_ini."' AND '".$datos->f_fin." 23:59:59'";
		// dd($sql);
		$articulos = $dbcon->qBuilder($dbcon->conn(), 'all', $sql);
		$spreadsheet = new Spreadsheet();
		$spreadsheet ->getProperties()->setCreator("LCDevelopersMx")->setTitle("Requisiciones");
		$spreadsheet->setActiveSheetIndex(0);
		$spreadsheet->getActiveSheet()->setTitle("Requisiciones");
		$hojaActiva = $spreadsheet->getActiveSheet();
		$hojaActiva->setCellValue('A1', 'Mayucsa');
		$hojaActiva->setCellValue('G1', date('Y/m/d'));
		...
		foreach ($articulos as $key => $value) {
			...
			$hojaActiva->setCellValue('G'.($key+4), explode(' ', $value->fecha_registro)[0]);
		}
		$hojaActiva->getColumnDimension('A')->setAutoSize(true);
		$hojaActiva->getColumnDimension('B')->setAutoSize(true);
		$hojaActiva->getColumnDimension('C')->setAutoSize(true);
		...
		$hojaActiva->getColumnDimension('G')->setAutoSize(true);
		$hojaActiva->getStyle('A:B')->getAlignment()->setHorizontal('left');
		$hojaActiva->getStyle('G')->getAlignment()->setHorizontal('right');
		$writer = new Xlsx($spreadsheet);
		$path = "../../../includees/archivos/excel/";
		if (!file_exists($path)) {
	        mkdir($path,0755,true);
	    }
	    $filename = $path."Requisiciones.xlsx";
		$writer->save($filename);
		dd($filename);
	}
	if ($tipo == 'entradasSalidas') {
		$sql = "";
		if ($datos->entradas) {
			$sql = "SELECT 'E' as tipo, cve_mov, cve_alterna, nombre_articulo, cantidad_entrada as cantidad, med.fecha_registro
			FROM movtos_entradas_detalle med
			INNER JOIN cat_articulos ca ON ca.cve_articulo = med.cve_articulo 
			WHERE  med.estatus_mov = 1 AND med.fecha_registro BETWEEN '".$datos->f_ini."' AND '".$datos->f_fin." 23:59:59' ";
		}
		if ($datos->entradas && $datos->salidas){
			$sql .= " UNION ";
		}
		if ($datos->salidas) {
			$sql .= "SELECT 'S' as tipo, cve_mov, cve_alterna, nombre_articulo, cantidad_salida as cantidad, msd.fecha_registro 
			FROM movtos_salidas_detalle msd
			INNER JOIN cat_articulos ca ON ca.cve_articulo = msd.cve_articulo
			WHERE msd.estatus_mov = 1 AND msd.fecha_registro BETWEEN '".$datos->f_ini."' AND '".$datos->f_fin." 23:59:59'
			";
		}
		$sql .= " ORDER BY fecha_registro";
		$movimientos = $dbcon->qBuilder($dbcon->conn(), 'all', $sql);
		$spreadsheet = new Spreadsheet();
		$spreadsheet ->getProperties()->setCreator("LCDevelopersMx")->setTitle("Movimientos");
		$spreadsheet->setActiveSheetIndex(0);
		$spreadsheet->getActiveSheet()->setTitle("Movimientos");
		$hojaActiva = $spreadsheet->getActiveSheet();
		$hojaActiva->setCellValue('A1', 'Mayucsa');
		$hojaActiva->setCellValue('F1', date('Y/m/d'));

		$hojaActiva->setCellValue('A3', 'Tipo');
		$hojaActiva->setCellValue('B3', 'Folio');
		$hojaActiva->setCellValue('C3', 'Clave artículo');
		$hojaActiva->setCellValue('D3', 'Descripción artículo');
		$hojaActiva->setCellValue('E3', 'cantidad');
		$hojaActiva->setCellValue('F3', 'fecha registro');

		foreach ($movimientos as $key => $value) {
			$hojaActiva->setCellValue('A'.($key+4), $value->tipo);
			$hojaActiva->setCellValue('B'.($key+4), $value->cve_mov);
			$hojaActiva->setCellValue('C'.($key+4), $value->cve_alterna);
			$hojaActiva->setCellValue('D'.($key+4), $value->nombre_articulo);
			$hojaActiva->setCellValue('E'.($key+4), $value->cantidad);
			$hojaActiva->setCellValue('F'.($key+4), explode(' ', $value->fecha_registro)[0]);
		}
		$hojaActiva->getColumnDimension('A')->setAutoSize(true);
		$hojaActiva->getColumnDimension('B')->setAutoSize(true);
		$hojaActiva->getColumnDimension('C')->setAutoSize(true);
		...
		$hojaActiva->getColumnDimension('E')->setAutoSize(true);
		$hojaActiva->getColumnDimension('F')->setAutoSize(true);
		$hojaActiva->getStyle('A:C')->getAlignment()->setHorizontal('left');
		$hojaActiva->getStyle('F')->getAlignment()->setHorizontal('right');
		$writer = new Xlsx($spreadsheet);
		$path = "../../../includees/archivos/excel/";
		if (!file_exists($path)) {
	        mkdir($path,0755,true);
	    }
	    $filename = $path."Movimientos.xlsx";
		$writer->save($filename);
		dd($filename);
	}
}
function Requisiciones($dbcon, $fechainicio, $fechafin, $tipoRANorm, $tipoRAAuto){
	$tipo = " tipo in ('N', 'A') ";
	if (!$tipoRANorm && $tipoRAAuto) {
		$tipo = " tipo = 'A'";
	}
	if ($tipoRANorm && !$tipoRAAuto) {
		$tipo = " tipo = 'N'";
	...
	INNER JOIN requisicion r ON r.cve_req = rd.cve_req 
	INNER JOIN cat_articulos ca ON ca.cve_articulo = rd.cve_art 
	WHERE ".$tipo." AND r.fecha_registro BETWEEN '".$fechainicio."' AND '".$fechafin." 23:59:59'";
	// dd($sql);
	$articulos = $dbcon->qBuilder($dbcon->conn(), 'all', $sql);
	$resultado = [];
	$aux = 0;
...
function entradasSalidas($dbcon, $datos){
	$sql = "";
	if ($datos->entradas) {
		$sql = "SELECT 'E' as tipo, cve_mov, cve_alterna, nombre_articulo, cantidad_entrada as cantidad, med.fecha_registro
		FROM movtos_entradas_detalle med
...
switch ($tarea){
	...
	case 'getExcel':
		...
		break;
	case 'Requisiciones':
		Requisiciones($dbcon, $objDatos->fechainicio, $objDatos->fechafin, $objDatos->tipoRANorm, $objDatos->tipoRAAuto);
		break;
	case 'entradasSalidas':
		entradasSalidas($dbcon, $objDatos);
		break;
}
